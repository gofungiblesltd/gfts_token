...
describe('GFTS token', function () {
  const totalSupplyAmount = ethers.BigNumber.from('1000')
  ...
  beforeEach(async function () {
    ...
    this.contract = await ethers.getContractFactory('GFTSToken')
    this.token = await this.contract.deploy(totalSupplyAmount)
  })

  it('has a name', async function () {
    expect(await this.token.name()).to.equal('GFTS Token')
  })

  it('has a symbol', async function () {
    expect(await this.token.symbol()).to.equal('GFTS')
  })
  ...
  describe('Checking token methods', function () {
    ...
    describe('Basic transfers', function () {
      ...
      it("spender's balance decreased", async function () {
        expect(await this.token.balanceOf(this.deployer.address)).to.equal(
          deployerGFTSBalance.sub(amount)
        )
      })

      it("receiver's balance increased", async function () {
        expect(await this.token.balanceOf(this.account1.address)).to.equal(
          amount
        )
      })
    ...
    describe('Approve', function () {
      ...
      it('reverts if ZERO_ADDRESS', async function () {
        await expect(
          this.token.connect(this.deployer).approve(ZERO_ADDRESS, amount)
        ).to.be.revertedWith('ERC20: approve to the zero address')
      })

      describe('transferFrom', function () {
        beforeEach(async function () {
          deployerGFTSBalance = await this.token.balanceOf(
            this.deployer.address
          )
        ...
        it('check account2 balance after transfer', async function () {
          expect(await this.token.balanceOf(this.account2.address)).to.equal(
            amount
          )
        })
        ...
        it('reverts if account1 has not enough allowance', async function () {
          ...
          await expect(transferFrom).to.be.revertedWith(
